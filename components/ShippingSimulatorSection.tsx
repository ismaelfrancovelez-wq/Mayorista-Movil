...
//
// Simulador interactivo de ahorro de envío por zona.
// Se muestra DEBAJO del área de compra en la página de producto.
// SOLO para compras fraccionadas (ya filtrado desde page.tsx con isFraccionado).
//
// Lógica "más lejano de base":
//   Cuando hay varias personas en el mismo código postal, el precio
//   que se divide entre todos es el del más lejano de la base
//   logística (Poeta Romildo Risso 3244). Esto cubre las diferencias
//   de distancia dentro de una misma zona.
//
// La API /api/lots/zone-companions ya aplica esta lógica y devuelve:
//   - shippingCostTotal: costo de referencia del usuario actual
//   - groups[].groupShippingCost: costo del más lejano en ese grupo
//   - groups[].buyerCount / buyerNames / postalCode
...
interface ZoneGroup {
  ...
  buyerCount: number;
  buyerNames: string[];
  groupShippingCost: number | null; // costo del más lejano del grupo
}
...
/* ====================================================
   HELPERS
==================================================== */
// Formato compacto para la tabla: $4K, $1K
function formatCompact(amount: number): string {
  if (amount >= 1000) return `$${Math.round(amount / 1000)}K`;
  return `$${amount.toLocaleString("es-AR")}`;
}

// Formato completo: $ 4.214
function formatFull(amount: number): string {
  return `$ ${amount.toLocaleString("es-AR")}`;
}

// Lista de nombres en español: "Juan", "Juan y Marta", "Juan, Marta y Carlos"
...
export default function ShippingSimulatorSection({ productId }: Props) {
  ...
  // Error de API: no mostrar nada para no romper la página
  if (apiError) return null;

  /* --------------------------------------------------
     Sin costo de envío (usuario sin dirección)
  -------------------------------------------------- */
  ...
  /* --------------------------------------------------
     ...
     shippingTotal = costo de referencia del usuario actual
     Se usa para la tabla y el slider
  -------------------------------------------------- */
  ...
  return (
    <div className="mt-8 border-t border-gray-100 pt-8">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <span className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm bg-white text-gray-800 font-medium">
          Tu zona de envío
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5 shadow-sm">
        ...
        {sliderValue > 1 && savings > 0 && (
          <p className="text-green-600 text-sm font-semibold">
            💚 Ahorros {formatFull(savings)} vs envío individual ({savingsPct}%
            menos)
          </p>
        )}
      </div>
      ...
      {data.groups.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
          ...
          {data.groups.map((group) => {
            // Costo que paga cada uno = costo del más lejano ÷ cantidad del grupo
            ...
            return (
              <div
                ...
                className="bg-white rounded-lg p-4 mb-3 last:mb-0 border border-blue-100"
              >
                ...
                {group.buyerCount === 1 && (
                  <div className="bg-blue-50 rounded-lg p-3 text-sm text-blue-700">
                    ...
                    pagaría aprox.{" "}
                    <strong>
                      {formatFull(Math.round(groupCost / 2))}
                    </strong>{" "}
                    de envío.
                  </div>
                )}
      ...
      {/* Hay compradores pero sin CP identificado */}
